...
from Bi_LSTM.modeling import Student_FT, AutoEncoder, Encoder, Decoder, BiLSTM
...
def do_eval(model, task_name, eval_dataloader, device, output_mode, eval_labels, num_labels):
    ...
    for batch_ in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            _, logits, _ = model(input_ids_lstm, seq_lengths_lstm.detach().cpu(), device)
...
def main():
    ...
    parser.add_argument('--hidden_size4lstm',
                        type=int,
                        default=600)
    parser.add_argument('--hidden_size4autoencoder',
                        type=int,
                        default=300)
    parser.add_argument('--autoencoder_train',
                        action='store_true',
                        help="Whether to train autoencoder.")
    parser.add_argument('--autoencoder_output_dir',
                        type=str,
                        default="")
    ...
    student_model = None
    if not args.autoencoder_train:
        bi_lstm = BiLSTM(args.embedding_size, args.hidden_size4lstm, num_labels, args.max_seq_len, n_layers=1)
        encoder = Encoder(args.hidden_size4lstm*2, args.hidden_size4autoencoder, n_layers=1)
        student_model = Student_FT(bi_lstm, encoder)
        student_model.to(device)
    else:
        encoder = Encoder(embedding_size=args.embedding_size, hidden_size=args.hidden_size4autoencoder, n_layers=1)
        decoder = Decoder(output_dim=args.embedding_size, embedding_size=args.embedding_size,
                          hidden_size=args.hidden_size4autoencoder, n_layers=1)
        student_model = AutoEncoder(encoder=encoder, decoder=decoder)
        student_model.to(device)

    if args.do_predict and not args.autoencoder_train:
        ...
        for epoch_ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration", ascii=True)):
                ...
                if not args.autoencoder_train:
                    student_hidden, student_logits, _= student_model(input_ids_lstm, seq_lengths_lstm.detach().cpu(), device)
                    teacher_autoencoder = torch.load(args.autoencoder_output_dir)
                    teacher_autoencoder.to(device)
                    
                    teacher_rep = teacher_rep.transpose(0, 1)
                    with torch.no_grad():
                        teacher_encoder_output, _ = teacher_autoencoder(teacher_rep, teacher_rep, device)
                    student_hidden_num = len(student_hidden)
                    teacher_encoder_output_num = len(teacher_encoder_output)

                    assert student_hidden_num % teacher_encoder_output_num == 0
                    ft_loss = loss_mse(teacher_encoder_output, student_hidden)

                    if output_mode == "classification":
                        # teacher_cls_loss = soft_cross_entropy(student_logits / args.temperature,
                    ...
                    tr_ft_loss += ft_loss.item()
                    # tr_teacher_cls_loss += teacher_cls_loss.item()
                else:
                    _, outputs = student_model(src=teacher_rep, trg=teacher_rep, device=device)
                    outputs = outputs.to(device)
                    loss = loss_mse(outputs, teacher_rep)
                ...
                if (global_step + 1) % args.eval_step == 0:
                    ...
                    result['ft_loss'] = ft_loss
                    result['loss'] = loss

                    result_to_file(result, output_eval_file)

                    if args.autoencoder_train:
                        save_model = True
                    else:
                        save_model = False
                    ...
                    if save_model:
                        ...
                        if not args.autoencoder_train:
                            ...
                            model_name = WEIGHTS_NAME
                            output_model_file = os.path.join(args.autoencoder_output_dir, model_name)
                            torch.save(model_to_save, output_model_file)
