        ...
        for epoch_ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration", ascii=True)):
                ...
                with torch.no_grad():
                    ...
                    teacher_rep = teacher_rep.flatten(start_dim=1, end_dim=2)


                _, student_flat_hidden, student_logits, student_reps = student_model(input_ids_lstm, seq_lengths_lstm.detach().cpu(), device)
                # # 最后一个fc前的输入
                # sp_student1 = torch.mm(student_flat_hidden, student_flat_hidden.t())
                # sp_student1 = F.normalize(sp_student1, dim=1)
                # sp_teacher1 = torch.mm(teacher_rep, teacher_rep.t())
                # sp_teacher1 = F.normalize(sp_teacher1, dim=1)
                # assert sp_student1.shape == sp_teacher1.shape
                # spkd_loss = loss_mse(sp_student1, sp_teacher1)
                
                # logits层
                sp_student = torch.mm(student_logits, student_logits.t())
                sp_student = F.normalize(sp_student, dim=1)
                sp_teacher = torch.mm(teacher_logits, teacher_logits.t())
                sp_teacher = F.normalize(sp_teacher, dim=1)
                assert sp_student.shape == sp_teacher.shape
                spkd_loss = loss_mse(sp_student, sp_teacher)
                ...
                loss = cls_loss + spkd_loss + tang_loss
                tr_cls_loss += cls_loss.item()
                tr_spkd_loss += spkd_loss.item()
